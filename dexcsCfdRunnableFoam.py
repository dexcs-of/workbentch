...
class CfdRunnable(QObject, object):
    def __init__(self, analysis=None, solver=None):
        ...
        if analysis and isinstance(analysis.Proxy, dexcsCfdAnalysis._CfdAnalysis):
            self.analysis = analysis
        else:
            if FreeCAD.GuiUp:
                self.analysis = dexcsCfdTools.getActiveAnalysis()
        ...
        if solver:
            ...
            if analysis:
                self.solver = dexcsCfdTools.getSolver(self.analysis)
            if not self.solver:
...
class CfdRunnableFoam(CfdRunnable):
    ...
    def get_solver_cmd(self, case_dir):
        ...
        cmd = dexcsCfdTools.makeRunCommand('./Allrun', case_dir, source_env=False)
    ...
    def getRunEnvironment(self):
        return dexcsCfdTools.getRunEnvironment()
