...
class _CommandCfdRunPlotWatcher:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "plotWatcher.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_RunPlotWatcher", "Run plotWatcher"),
                'Accel': "S, P",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_RunPlotWatcher", _("Run plotWatcher"))}
    ...
    def Activated(self):
        ...
        for i in members:
            if isinstance(i.Proxy, _CommandCfdRunPlotWatcher):
                FreeCADGui.activeDocument().setEdit(i.Name)
                isPresent = True

        # Allowing user to re-create if CFDSolver was deleted.
        if not isPresent:
            #FreeCADGui.runCommand('Std_Macro_13',0)
            #import runPlotWatcher
