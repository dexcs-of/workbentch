...
class _CommandCfdRunParaview:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "paraview.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_RunParaview", "Run paraview"),
                'Accel': "S, P",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_RunParaview", _("Run paraview"))}
    ...
    def Activated(self):
        ...
        for i in members:
            if isinstance(i.Proxy, _CommandCfdRunParaview):
                FreeCADGui.activeDocument().setEdit(i.Name)
                isPresent = True

        # Allowing user to re-create if CFDSolver was deleted.
        if not isPresent:
            #FreeCADGui.runCommand('Std_Macro_2',0)
            #import runParaview
