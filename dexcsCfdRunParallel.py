...
class _CommandCfdRunParallel:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "parallel.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_RunParallel", "Run parallel"),
                'Accel': "S, P",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_RunParallel", _("Run parallel"))}
    ...
    def Activated(self):
        ...
        for i in members:
            if isinstance(i.Proxy, _CommandCfdRunParallel):
                FreeCADGui.activeDocument().setEdit(i.Name)
                isPresent = True

        # Allowing user to re-create if CFDSolver was deleted.
        if not isPresent:
            #FreeCADGui.runCommand('Std_Macro_14',0)
            #import runParalel
