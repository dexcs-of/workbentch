...
## 更新 2026/4/21（v0.31）
CfdOFを修正するパッチデータとスクリプトを同梱
但し、CfdOFのバージョンは1.34.12〜18　on FreeCAD 1.1.0
（CfdOFのバージョンが異なる場合の動作検証はできていない）

## 更新 2026/4/10（v0.30）
TreeFoamサブセットを、タスク画面から呼び出せるようにした事に伴う諸々変更

## 更新 2026/4/6（v0.29）
FreeCAD最新版（v1.1）における動作不具合を解消。
旧版（v1.0）でも動作OK（のはずだが、十分にテストできていない）。
