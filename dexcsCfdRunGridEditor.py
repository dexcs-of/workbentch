...
class _CommandCfdRunGridEditor:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "gridEditor.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_RunGridEditor", "Run gridEditor"),
                'Accel': "S, P",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_RunGridEditor", _("Run gridEditor"))}
    ...
    def Activated(self):
        ...
        for i in members:
            if isinstance(i.Proxy, _CommandCfdRunGridEditor):
                FreeCADGui.activeDocument().setEdit(i.Name)
                isPresent = True

        # Allowing user to re-create if CFDSolver was deleted.
        if not isPresent:
            #FreeCADGui.runCommand('Std_Macro_1',0)
            #import runGridEditor
