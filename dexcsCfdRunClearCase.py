...
class _CommandCfdRunClearCase:
    def GetResources(self):
        icon_path = os.path.join(dexcsCfdTools.get_module_path(), "Gui", "Resources", "icons", "iniCase.png")
        return {'Pixmap': icon_path,
                'MenuText': QtCore.QT_TRANSLATE_NOOP("Cfd_RunClearCase", "Run clear case"),
                'Accel': "S, P",
                'ToolTip': QtCore.QT_TRANSLATE_NOOP("Cfd_RunClearCase", _("Run clear case"))}
    ...
    def Activated(self):
        ...
        for i in members:
            if isinstance(i.Proxy, _CommandCfdRunClearCase):
                FreeCADGui.activeDocument().setEdit(i.Name)
                isPresent = True

        # Allowing user to re-create if CFDSolver was deleted.
        if not isPresent:
            #FreeCADGui.runCommand('Std_Macro_4',0)
            #import runClearCase
